/*
  ...
  5) Arduino Mega (by using a voltage converter); 
  6) Pi Pico; --> this code    
  ...
  ATTENTION: 
  SI473X bases device  and Pi Pico based device wire up. 
...
#define BFO_SWITCH 14   //#### pico,  A0 (Alternative to the pin 13). Used to select the enconder control (BFO or VFO)
...
*/
Band band[] = {
  {FM_BAND_TYPE, 7600, 10800, 9150, 10},
  {LW_BAND_TYPE, 100, 510, 300, 1},
  {MW_BAND_TYPE, 522, 1710, 747, 9},
  ...
  {SW_BAND_TYPE, 10000, 11000, 10100, 5}, // 30 meters
  {SW_BAND_TYPE, 11200, 12500, 11940, 5},
  {SW_BAND_TYPE, 13400, 13900, 13600, 5},
  {SW_BAND_TYPE, 14000, 14500, 14200, 5}, // 20 meters
  {SW_BAND_TYPE, 15000, 15900, 15300, 5},
  {SW_BAND_TYPE, 17200, 17900, 17600, 5},
  {SW_BAND_TYPE, 18000, 18300, 18100, 5},  // 17 meters
  {SW_BAND_TYPE, 21000, 21900, 21200, 5},  // 15 mters
  {SW_BAND_TYPE, 24890, 26200, 24940, 5},  // 12 meters
  {SW_BAND_TYPE, 26200, 27900, 27500, 5},  // CB band (11 meters)
  {SW_BAND_TYPE, 28000, 30000, 28400, 5}
}; // 10 meters

uint16_t fmstation[] = {9150,9040,8520,8250,8040,7810}; //####  pico  change maxfmidx if expand
int fmidx = 0;                                          //####  pico
...
bool preamponoff = false;
bool lpreamponoff = false;
long elapsedButton2 = millis();
...
void setup()
{
  ...
  digitalWrite(25,HIGH); //####  pico Turn on LED
  Serial.begin(9600);
  // Encoder pins
  ...
  Wire.setSDA(20);                       //#### pico I2C
  Wire.setSCL(21);                       //#### pico I2C
  ...
  //attachInterrupt(digitalPinToInterrupt(20), preamp_on_off, FALLING); // Pre AMP on/off
  ...
  si4735.setFMDeEmphasis(1); // 50 in Japan
...
void loop()
{
  ...
  if ((millis() - elapsedButton) > MIN_ELAPSED_TIME)
  {
    ...
    else if (digitalRead(STEP_SWITCH) == LOW)
    {
      if ( currentMode == FM) {
        fmidx ++;                               //#### pico
        if (fmidx > maxfmidx) fmidx = 0;        //#### pico
        currentFrequency = fmstation[fmidx];    //#### pico
        previousFrequency = currentFrequency;   //#### pico
        si4735.setFrequency(fmstation[fmidx]);  //#### pico
        showStatus();                           //#### pico
        //fmStereo = !fmStereo;   //#### pico comment out old codes
        //if ( fmStereo )
